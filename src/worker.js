// Haupt-Worker: liefert die statische Website (aus /public) aus und
// übernimmt zusätzlich die Forum-API unter /api/forum.
//
// WICHTIG: Damit das Forum Daten speichern kann, muss im Cloudflare-Dashboard
// unter deinem Projekt -> Reiter "Bindings" -> "Add" -> "KV namespace"
// eine Bindung mit dem Variablennamen FORUM_KV hinzugefügt werden
// (Namespace vorher unter "Storage & Databases" -> "KV" anlegen).
...
async function handleForumGet(request, kv) {
...
async function handleForumPost(request, kv) {
  let body;
  try {
  ...
  if (body.action === 'newThread') {
    ...
    const threadDoc = { id, title, category, createdAt: now, lastActivity: now, posts: [firstPost] };
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === '/api/forum') {
      if (!env.FORUM_KV) {
        return json({ error: 'Der Forum-Speicher ist noch nicht verbunden. Bitte im Cloudflare-Dashboard unter „Bindings" eine KV-Namespace namens FORUM_KV hinzufügen.' }, 500);
      }
      if (request.method === 'GET') return handleForumGet(request, env.FORUM_KV);
      if (request.method === 'POST') return handleForumPost(request, env.FORUM_KV);
